...
export default {
  ...
  async mounted() {
    ...
    const res3 = await fetch(`/api/items/?category=${this.$route.params.subCataId}`, {
      method: "GET",
    });
    console.log("fetch");
    console.log(res3);
    const res4 = await res3.json();
    this.allItems = res3.data.items;
    const items = res3.data.items;
    /*
