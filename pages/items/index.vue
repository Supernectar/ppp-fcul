...
export default {
	...
	async mounted() {
		...
		const res1 = await fetch(
			`/api/categories/${this.$route.query.superId}`,
			{
				method: 'GET'
			}
		);
		...
		const res3 = await fetch(
			`/api/categories/${this.$route.query.cateId}`,
			{
				method: 'GET'
			}
		);
		...
		const res5 = await fetch(
			`/api/categories/${this.$route.query.subId}`,
			{
				method: 'GET'
			}
		);
		...
		const res7 = await fetch(
			`/api/items?category=${this.$route.query.subId}`,
			{
				method: 'GET'
			}
		);
