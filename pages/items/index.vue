      ...
      <div class="container mx-auto grid grid-cols-12 gap-8">
        <div class="m-4 bg-white p-4 rounded bg-light col-span-3">
          ...
          >
            ...
            />
            <label class="form-check-label" :for="'flexCheckDefault' + aa"> aa </label>
            <br />
          </div>
        </div>
        <!--
        ...
        <div class="col-span-9">
          <p>
            {{ this.$route.query.superName }}
            >
            {{ this.$route.query.cateName }}
            >
            {{ this.$route.query.subName }}
          </p>
          <div v-for="item in items" :key="item._id" class="col">
            <div class="block p-6 rounded-lg shadow-lg bg-white max-w-sm">
              <img src="https://picsum.photos/600/300/?image=25" class="rounded-t-lg" />
...
export default {
  ...
  async mounted() {
    ...
    if (this.filters.length != 0) {
      result = this.items.filter((el) => this.filters.includes(el.attributes.brand));
    }
