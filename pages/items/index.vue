	...
	<div class="h-screen">
		...
		<div class="row">
			<div class="container mx-auto grid grid-cols-12 gap-8">
				<div
					class="m-4 bg-white p-4 rounded bg-light col-span-3"
				>
					<h5>FILTROS</h5>
					{{ this.attributes }}
					<br />
...
export default {
	...
	async mounted() {
		...
		const res1 = await fetch(
			`/api/categories/${this.$route.params.superId}`,
			{
				method: 'GET'
			}
		);
		...
		console.log('a ' + attr_supercate);
		if (attr_supercate.length != 0) {
			this.attributes.push.apply(
				this.attributes,
				attr_supercate
			);
		}
		...
		const res3 = await fetch(
			`/api/categories/${this.$route.params.cateId}`,
			{
				method: 'GET'
			}
		);
		...
		const res5 = await fetch(
			`/api/categories/${this.$route.params.subId}`,
			{
				method: 'GET'
			}
		);
		...
		for (let a = 0; a < this.attributes.length; a++) {
			for (let i = 0; i < this.items.length; i++) {
				if (
					this.items[i].attributes.includes(
						this.attributes[a]
					)
				) {
					this.attributes.splice(
						a,
						0,
						this.items[i].attributes.a
					);
				}
			}
		}
		console.log('aaaaaaaaaaa' + this.attributes);
