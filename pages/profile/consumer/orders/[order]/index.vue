...
let itemIds = [];
const res = await fetch(`/api/users/${store.user.userId}/orders/${route.params.order}`);
...
for (let i = 0; i < itemIds.length; i++) {
  let res2 = await fetch(`/api/items/${itemIds[i]}`);
  let resJson2 = await res2.json();
  console.log(resJson2);
  let resCat = await fetch(`/api/categories/${resJson2.data.items[0].category}`);
  let resCatJson = await resCat.json();
