		...
		<div class="flex h-[calc(100vh-4rem)] items-center">
			<div class="w-100 m-auto bg-white rounded p-4">
				...
				<FormKit type="form" @submit="RegisterUser">
					<FormKit label="Email address" v-model="email" type="email" help="What email should we use?"
						validation="required|email|emailIsRegistered" :validation-rules="{
							emailIsRegistered
						}" :validation-messages="{
	emailIsRegistered:
		'This email is already registered'
}" outer-class="mb-4" label-class="form-label inline-block mb-2 text-gray-700"
					...
					<FormKit label="Password" v-model="password" type="password" help="Choose an account password"
					...
					<FormKit v-model="terms" type="checkbox" help="Do you agree to our terms of service?" name="terms"
						validation="required|accepted" outer-class="mb-4"
						...
						<template #label="context">
							I agree with the
							<a href="terms-and-conditions.pdf" download="clearchoice_terms_and_conditions"
								class="text-blue-600 hover:text-blue-700 transition duration-300 ease-in-out mb-4">terms
								of
								service</a>
						</template>
					</FormKit>
				...
				<FormKit type="button" label="Register with Google"
...
async function RegisterUser() {
	...
	router.push('/register/configure');
}
